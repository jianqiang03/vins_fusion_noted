...
{
    inputImageCnt++;
    map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> featureFrame; // 当前帧的图像feature
    ...
    {
        mBuf.lock();
        featureBuf.push(make_pair(t, featureFrame)); // 加入队列
        mBuf.unlock();
        TicToc processTime;
        processMeasurements(); 
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    {
        ROS_INFO("calibrating extrinsic param, rotation movement is needed"); // 标定旋转外参，运动需要有旋转
        if (frame_count != 0)
        {
            //得到两帧之间归一化特征点
        ...
        if (!STEREO && USE_IMU)
        {
            ...
            if (frame_count == WINDOW_SIZE)
            {
                ...
                if(result) //初始化成功
                {
                    ...
                    ROS_INFO("initialize costs: %fms", t_init.toc());
                }
                else
                    slideWindow();
            }
...
/**
 * @brief   判断两帧有足够视差>30且内点数目大于12则可进行初始化，同时得到R和T
 ...
 * @param[out]   l 保存滑动窗口内与当前帧满足初始化条件的那一帧
