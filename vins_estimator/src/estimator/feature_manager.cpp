 ...
 * @param image image 某帧所有特征点的[camera_id,[x,y,z,u,v,vx,vy]]s构成的map,索引为feature_id
 * @param td td IMU和cam同步时间差
 * @return true 次新帧是关键帧
 * @return false 非关键帧
 */
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    for (auto &id_pts : image)
    {
        ...
        // id_pts.second[0] 表示左目
        // id_pts.second[0] 表示右目
        ...
        else if (it->feature_id == feature_id)
        {
            ...
            last_track_num++;
            //last_track_num，表示该特征点被四帧或以上观测到
    ...
    // 关键帧 窗口里只有一帧，或跟踪到的特征点小于20，或被多帧观测到的特征点个数小于40， 或新的特征点数大于追踪到的特征点数的一半
